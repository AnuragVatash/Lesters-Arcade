...
export default function NumberFinder({ user }: NumberFinderProps) {
    ...
    const { oracleActive, resetOracle } = useOracle({
        ...
        onOracleActivated: () => {
            ...
            if (gameBoard.length > 0) {
                ...
                console.log('Moving cursor from:', hoveredRow, hoveredCol, 'to:', foundRow, foundCol);
                if (foundRow !== -1) {
    ...
    const startGame = () => {
        ...
        } while (randomCursorRow === newRowIndex && randomCursorCol === newColIndex);
    ...
    if (gameStarted && gameBoard.length > 0) {
        ...
        for (let i = 1; i <= 3; i++) {
            ...
            displayedNumbers.push(gameBoard[nextHighlightRow][nextHighlightCol].toString());
        }
    ...
    const handleSubmit = useCallback(() => {
        ...
        if (isCorrect) {
            ...
            if (gameStartTime && user) {
                ...
                const comparison = submitLeaderboardTime(user.username, gameTime, 'numberFinder', user.isGuest);
    ...
    return (
        <div className="relative h-full overflow-hidden">
            ...
            <img src={'/numberFinder/fullPic.png'} className="w-full" alt="Number Finder Game" />
