...
function ParticleSystem({
  ...
  const createParticle = useCallback(
    (config: Partial<ParticleConfig> = {}): Particle => {
  ...
  useEffect(() => {
    if (canvasRef.current) {
      ...
      const c = canvasRef.current as CanvasWithParticles;
      c.addParticle = addParticle;
      c.clearParticles = clearParticles;
      c.getParticleCount = getParticleCount;
    }
...
export const useParticleSystem = () => {
  ...
  const createTrailEffect = useCallback(
    (x: number, y: number, vx: number, vy: number, count?: number) => {
      createTrail(x, y, vx, vy, count, addParticle);
    },
    [addParticle]
  );

  const createMatrixRainEffect = useCallback(
  ...
  const createGlowEffectCallback = useCallback(
